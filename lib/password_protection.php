...
function italystrap_password_protection_post_page() {
    ...
    $form = '<div class="alert alert-danger" role="alert">
        <form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" method="post" class="form-inline" role="form">
        ' . __( 'To view this protected post, enter the password below:', 'ItalyStrap' ) . '
            <div class="form-group  has-error">
                <label for="' . $label . '">' . __( 'Password: ', 'ItalyStrap' ) . ' </label><input name="post_password" id="' . $label . '" type="password"  class="form-control" size="20" maxlength="20"  placeholder="' . __( 'Enter password', 'ItalyStrap' ) . '"><input type="submit"  class="btn btn-danger" name="Submit" value="' . esc_attr__( "Submit" ) . '" />
            </div>
        </form>
    </div>
    ';
